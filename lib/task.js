...
var addRepository = exports.addRepository = function*(repository, branch, hosts, provider, url, preCommands, options, processCommand, confFile) {
  processCommand = processCommand || misc.processCommand;
  ...
  try {
    ...
    if (err.message !== util.format("fatal: destination path '%s' already exists and is not an empty directory.", repoDir) &&
        err.message !== util.format("fatal: repository '%s' does not exist", repoDir)) {
      throw err;
    }
  }
  ...
  var confPath = ['providers', provider, repository, branch];
  var project = {};

  try { // Project already exists in config file
    project = ramble(conf, confPath);
    project.hosts = project.hosts.concat(hosts);
    project.options = options ? options : project.options;
  ...
  ramble(conf, confPath, project, true);
